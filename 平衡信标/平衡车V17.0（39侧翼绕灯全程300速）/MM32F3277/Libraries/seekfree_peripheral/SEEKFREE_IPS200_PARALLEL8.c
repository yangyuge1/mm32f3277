...
uint8  out    [100      ] [MT9V03X_W] = {0};//水平灰度分析输出图
uint8  out_1  [MT9V03X_H] [100      ] = {0};//垂直灰度分析输出图
...
//	if((start_y+dis_height)>IPS200_Y_MAX)	ZF_ASSERT(0);
    ...
    for(j=0;j<dis_height;j++)
    {
        for(i=0;i<dis_width;i++)
        {
					if( j*height/dis_height==y || i*width/dis_width==x )
					{
						ips200_wr_data16(RED);
					}
					else if(j*height/dis_height==bounder|| i*width/dis_width==aim)
					{
...
void analyse(const uint8 *p,int width,int height,int x_top,int x_bottom,int y_top,int y_bottom,uint8 *out ,int x_max,int y_max,int start_x,int start_y)
{
	...
	{	
		...
		{
			if(y>=(1-data[x/3]/sum)*y_max)
			*(out+y*x_max+x)=255;
			else
			*(out+y*x_max+x)=0;
		}
	}
	ips200_displayimage032_zoom1(out,x_max,y_max,start_x,start_y,x_max,y_max/2);
}
//-------------------------------------------------------------------------------------------------------------------
//  @brief      剖面灰度分布分析
//  @param      *p      需要分析的图像
...
void dissection_y(const uint8 *p,uint16 width,uint16 height,uint16 y, uint16 dis_width, uint16 dis_height,uint16 start_x,uint16 start_y)
{
			for(int i=0;i<width;i++)
			{
					...
					{
						if(j>=(1-*(p+y*width+i)*1.0/255.0)*100)
						out[j][i]=255;
						else
						out[j][i]=0;
					}
			}
	ips200_displayimage032_zoom1(out[0],MT9V03X_W,100,start_x,start_y,dis_width,dis_height);
}
...
void dissection_x(const uint8 *p,uint16 width,uint16 height,uint16 x, uint16 dis_width, uint16 dis_height,uint16 start_x,uint16 start_y)
{
		...
		{	
			for(int i=0;i<100;i++)
			{
						if(i>=(1-*(p+j*width+x)*1.0/255.0)*100)
						out_1[j][i]=255;
						else
						out_1[j][i]=0;
			}
		}
	ips200_displayimage032_zoom1(out_1[0],100,MT9V03X_H,start_x,start_y,dis_width,dis_height);
}